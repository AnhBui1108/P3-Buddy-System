 ...
 * Allocate several blocks, frees them
 */
void test_multiple_allocation(void){
  fprintf(stderr, "->Test allocating and freeing multiple times\n");
  struct buddy_pool pool;
  size_t size = UINT64_C(1) << DEFAULT_K;
  buddy_init(&pool, size);

  //Add 3 blocks
  void *block1 = buddy_malloc(&pool, 128);
  void *block2 = buddy_malloc(&pool, 253);
  ...
  //free blocks in the different order
...
/**
 * Test buddgy_realloc expanding a block
 * allocates a small block, writes data into it, then realloctes to a large block
 * it checks if the original data is preserved.
 */

 void test_buddy_realloc_expend(void){
  fprintf(stderr, "-> Testing buddy_realloc to expand a block\n");

  struct buddy_pool pool;
  size_t size = UINT64_C(1) << DEFAULT_K;
  buddy_init(&pool, size);

  //Allocate a block of 100 bytes and fill it with a known pattern
  char *ptr = buddy_malloc(&pool, 100);
    assert(ptr != NULL);
    memset(ptr, 0xAA, 100);

    // Expand the allocated block to 300 bytes.
    char *new_ptr = buddy_realloc(&pool, ptr, 300);
    assert(new_ptr != NULL);

    // Verify that the original data (first 100 bytes) remains unchanged.
    for (int i = 0; i < 100; i++) {
        assert(new_ptr[i] == (char)0xAA);
    }
 ...
 /**
 * Test buddy_realloc shrinking a block.
 * Allocates a block that is larger than needed, writes data into it, then shrinks it
 * and verifies that the beginning of the block is preserved.
 */
void test_buddy_realloc_shrink(void)
{
    fprintf(stderr, "->Testing buddy_realloc to shrink a block\n");
    struct buddy_pool pool;
    size_t size = UINT64_C(1) << DEFAULT_K;
    buddy_init(&pool, size);

    // Allocate a block of 300 bytes and fill it with a known pattern.
    char *ptr = buddy_malloc(&pool, 300);
    assert(ptr != NULL);
    memset(ptr, 0xBB, 300);

    // Shrink the allocated block to 150 bytes.
    char *new_ptr = buddy_realloc(&pool, ptr, 150);
    assert(new_ptr != NULL);

    // Verify that the first 150 bytes remain unchanged.
    for (int i = 0; i < 150; i++) {
        assert(new_ptr[i] == (char)0xBB);
    }
...
int main(void) {
  ...
  RUN_TEST(test_buddy_realloc_expend);
